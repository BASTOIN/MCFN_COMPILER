...
export function activate(context: vscode.ExtensionContext) {
  ...
  function snippet(label: string, detail: string, body: string) {
  ...
  snippetItems.push(
    snippet("func", "함수 스니펫", "func ${1:name}(){\n\t$0\n}"),
    snippet("obj", "오브젝트 선언", "obj ${1:objective}(${2:dummy});"),
    snippet("var", "변수 초기화", "var ${1:objective}:${2:name};"),
    snippet("const", "상수", "const ${1:NAME} = ${2:10};"),
    snippet("if", "if 블록", "if(${1:a}:${2:x} ${3:==} ${4:10}){\n\t$0\n}"),
    ...
    snippet("call", "함수 호출", "call ${1:foo}();"),
    snippet("call[Q]", "호출 + 강한 대기(wait)", "call ${1:foo}()[${2:Q}];"),
    snippet("run", "명령 실행", "run(\"${1:say hi}\");"),
    snippet("runv", "보간/JSON tellraw", "run(v\"${1:tellraw @a {text:\\\"[${2:obj}:${3:name}]\\\"}}\"\");"),
    snippet("show", "간단 출력", "show(v\"${1:hello}\");"),
    snippet("title", "타이틀", "title(title, \"${1:text}\");"),
    snippet("rand", "난수", "rand(${1:a}:${2:x}, ${3:0}, ${4:100});"),
    snippet("assign", "대입/연산", "${1:a}:${2:x} = ${3:b}:${4:y} ${5:+} ${6:c}:${7:z};")
  );

  // 문서에서 obj/var 스캔 → 제안에 쓰기
  function scanDoc(doc: vscode.TextDocument) {
    const text = doc.getText();
    const objSet = new Set<string>();
    const nameMap = new Map<string, Set<string>>(); // objective -> names
    ...
    while ((m = varRe.exec(text))) {
      ...
      for (const p of pairs) {
        ...
        if (!nameMap.has(obj)) nameMap.set(obj, new Set());
